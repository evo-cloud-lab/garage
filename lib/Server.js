...
var Server = Class({
    constructor: function (opts) {
        ...
        this.app.get('/clusters', this.getClusters.bind(this));
        this.app.get('/clusters/:cluster/nodes', this.getClusterNodes.bind(this));
        this.app.get('/clusters/:cluster/nodes/:node', this.getClusterNode.bind(this));
        this.app.post('/clusters', this.addCluster.bind(this));
        this.app.post('/clusters/reload', this.reloadClusters.bind(this));
        this.app.post('/clusters/:cluster/nodes/:node/start', this.startNode.bind(this));
        this.app.post('/clusters/:cluster/nodes/:node/stop', this.stopNode.bind(this));
        this.app.post('/clusters/:cluster/start', this.startCluster.bind(this));
        this.app.post('/clusters/:cluster/stop', this.stopCluster.bind(this));
    ...
    addCluster: function (req, res) {
        var dir = req.body.path;
        if (!dir) {
            this.error(res, 400, new Error('Bad request'));
        } else {
            this.manager.add(dir, function(err, cluster) {
                err ? this.error(res, 500, err) : this.result(res, cluster.info, 201);
            }.bind(this));
        }
    },
    ...
    startNode: function (req, res) {
        new Node(req.cluster, req.params.node).start(function (err) {
            err ? this.error(res, 500, err) : this.result(res);
        }.bind(this));
    },

    stopNode: function (req, res) {
        new Node(req.cluster, req.params.node).stop(function (err) {
            err ? this.error(res, 500, err) : this.result(res);
        }.bind(this));
    },
    ...
    startCluster: function (req, res) {
        ...
        if (Array.isArray(requestIds)) {
            ...
            async.each(ids, function (id, next) {
                new Node(req.cluster, id).start(next);
            }, function (err) {
                err ? this.error(res, 500, err) : this.result(res);
            }.bind(this));
        } else {
